...
/*
connect_new_recurse(Vltl_sast_operation *connect_me, Vht inuse_above, Vht inuse_below_here){
    // Manage what registers are inuse here
    Vht<Vltl_global_register, bool>> inuse_from_here = init();
    Vht<Vltl_global_register, bool>> inuse_below_child = init();
    copy inuse_above into inuse_from_here

    // set registers in use based on what this is
    switch(connect_me->kind) {
    case VLTL_SAST_OPERATION_KIND_RETURN:
        inuse_below_here[rax] = yes;
    case VLTL_SAST_OPERATION_KIND_DIV:
        inuse_below_here[rax] = yes;
        inuse_below_here[rdx] = yes;
    case VLTL_SAST_OPERATION_KIND_FUNCTION:
        const Vltl_asm_register registers_in_use_here[6] = { rdi, rsi, rcx, rdx, r8?, r9? };
        for(size_t i = 0; i < argc(*connect_me); i++) {
            inuse_from_here[registers_in_use_here[i]] = true;
        }

        for registers in clobbered_by_function_call {
            set them ig
        }
    }

    if(connect_me->destination == unset) {
        get free register using inuse_above;
    }

    for(size_t i = 0; i < argc(connect_me); i++) {
        connect_new_recurse(connect_me->arguments[i], inuse_from_here, inuse_below_child);
        inuse_from_here[child->destination] if type(child->destination) == register;
        inuse_below_child[child->destination] if type(child->destination) == register;

        for(reg, val : inuse_below_child) {
            if(val == false || reg in inuse_below_here) {
                continue;
            }

            bool done = false;
            for(size_t j = i; !done; j--) {
                if(j == 0) {
                    done = true;
                }

                if(reg == connect_me->arguments[j]->destination) {
                    protect(connect_me->arguments[j]->destination);
                }
            }

            inuse_below_here[reg] = true;
        }

        clear(inuse_below_child);
    }
